...
const gatUserBookingInToDb = async(token: string) =>{
    ...
    const result = await Booking.find({user: user._id, isDelete: false}).populate("faculty")
    if(!result){
...
const deleteUserBooking = async(id: string, token: string) =>{
    ...
    if(checkBooking.isDelete){
        throw new AppError(httpStatus.NOT_FOUND, "this booking is all ready delete")
    }
    const result = await Booking.findByIdAndUpdate(id, {isDelete: true}, {new: true})
    return result 
}
